...
func (s *MovieService) FetchAndStoreMovies() ([]types.Movie, error) {
	...
	results := make(chan types.FetchResult, len(movieSources)) //buffered channel
	...
	for id, source := range movieSources {
		...
		go func(movieID string, apiSource string) {
			...
			default:
				data = fmt.Sprintf("Unknown source for %s", movieID)
			}
	...
	for res := range results {
		movie, err := mapToMovieStruct(res)
		if err != nil {
			fmt.Printf("Failed to map movie %s: %v\n", res.ID, err)
			continue
		}
